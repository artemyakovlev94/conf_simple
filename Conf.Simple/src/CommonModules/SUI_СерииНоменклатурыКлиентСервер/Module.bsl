...
Функция ПредопределенныйАлгоритмСозданияСерииНоменклатуры(КонфигурацияНастройки) Экспорт
	
	Алгоритм = "";
	
	Если КонфигурацияНастройки = ПредопределенноеЗначение("Перечисление.SUI_ПредопределенныеНастройкиПодсистемы.УправлениеТорговлей11") Тогда
		Алгоритм = "
		|Запрос = Новый Запрос;
