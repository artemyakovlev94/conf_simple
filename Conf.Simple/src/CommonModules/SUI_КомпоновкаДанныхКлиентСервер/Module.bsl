...
Функция ПолучитьРазличныеЭлементыОтбора(ИсточникОтбор, ЭлементСравненияОтбор) Экспорт
	
	Результат = Новый Структура("ЭлементыИсточник, ЭлементыСравнения", Новый СписокЗначений(), Новый СписокЗначений());
	...
	Для Каждого ЭлементОтбора Из ИсточникОтборЭлементы Цикл
		...
		Если Не ЕстьТакойЖеОтбор(ЭлементОтбора, ЭлементСравненияОтборЭлементы, НайденныйЭлемент) Тогда
			Результат.ЭлементыИсточник.Добавить(НайденныйЭлемент);
		Иначе
			
			Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			
				РезультатГруппы = ПолучитьРазличныеЭлементыОтбора(ЭлементОтбора.Элементы, НайденныйЭлемент);
				
				Если РезультатГруппы <> Неопределено Тогда
					
					Для Каждого ЭлементГруппыИсточник Из РезультатГруппы.ЭлементыИсточник Цикл
						Результат.ЭлементыИсточник.Добавить(ЭлементГруппыИсточник.Значение);
					КонецЦикла;
					
					Для Каждого ЭлементГруппыСравнения Из РезультатГруппы.ЭлементыСравнения Цикл
						Результат.ЭлементыСравнения.Добавить(ЭлементГруппыСравнения.Значение);
					КонецЦикла;
	...
	Для Каждого ЭлементОтбора Из ЭлементСравненияОтборЭлементы Цикл
		...
		Если НЕ ЕстьТакойЖеОтбор(ЭлементОтбора, ИсточникОтборЭлементы, НайденныйЭлемент) Тогда
			Результат.ЭлементыСравнения.Добавить(НайденныйЭлемент);
		Иначе
			...
			Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
				...
				Если РезультатГруппы <> Неопределено Тогда
					...
					Для Каждого ЭлементГруппыИсточник Из РезультатГруппы.ЭлементыИсточник Цикл
						Результат.ЭлементыСравнения.Добавить(ЭлементГруппыИсточник.Значение);
					КонецЦикла;
					
					Для Каждого ЭлементГруппыСравнения Из РезультатГруппы.ЭлементыСравнения Цикл
						Результат.ЭлементыИсточник.Добавить(ЭлементГруппыСравнения.Значение);
					КонецЦикла;
...
//
Функция НайтиПоле(Структура, Поле, ВключаяВложенныеПоля = Ложь) Экспорт
	...
	Для Каждого ЭлементОтбораСравнения Из ЭлементыОтбора Цикл
		...
		Если ТипЗнч(ЭлементОтбораСравнения) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			Если ТипЗнч(ЭлементОтбораСравнения.ПравоеЗначение) = Тип("СписокЗначений")
				...
				ПравыеЗначенияРавны = SUI_ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(
