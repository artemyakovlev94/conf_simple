...
Функция ПолучитьИменаКолонокРезультатаЗапроса(РезультатЗапроса, ИсключитьКолонки = "", ВернутьСтрокой = Истина) Экспорт
	...
	ИменаКолонок = SUI_ОбщегоНазначенияКлиентСервер.РазностьМассивов(ИменаКолонок, 
...
Функция КолонкиРезультатаЗапросаВСтруктуру(РезультатЗапроса, Знач СписокКолонок = "", Знач ИсключитьКолонки = "", 
	...
	Если СписокКолонок.Количество() > 0 Тогда
		СписокКолонок = SUI_ОбщегоНазначенияКлиентСервер.РазностьМассивов(СписокКолонок, ИсключитьКолонки);
	...
	Если ВключитьНачинаетсяС.Количество() > 0 Тогда
		ВключитьНачинаетсяС = SUI_ОбщегоНазначенияКлиентСервер.РазностьМассивов(ВключитьНачинаетсяС, ИсключитьНачинаетсяС);
...
Функция ПредопределенныйЭлемент(ПолноеИмяПредопределенного) Экспорт
	...
	ПредопределенныеЗначения = ОписаниеТипаФиксированноеСоответствие.ПривестиЗначение(
		SUI_СтандартныеПодсистемыПовтИсп.СсылкиПоИменамПредопределенных(ПоляПредопределенного.ПолноеИмяОбъектаМетаданных));
...
Функция ЭтоСсылка(ПроверяемыйТип) Экспорт
	...
	Иначе
		ПроверяемыеТипы = Новый ОписаниеТипов(SUI_ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПроверяемыйТип));
	КонецЕсли;
...
Функция СжатьСтроку(Строка) Экспорт
	...
	Если ПустаяСтрока(ОписаниеТипаСтрока(0).ПривестиЗначение(Строка)) Тогда
		Возврат "";
	Иначе
		Возврат XMLСтрока(Новый ХранилищеЗначения(Строка, Новый СжатиеДанных(9)));
	КонецЕсли;
...
// 
// Параметры:
//  Строка - Строка - Строка сжатая функцией см. SUI_ОбщегоНазначения.СжатьСтроку
// 
// Возвращаемое значение:
...
Функция ВосстановитьСжатуюСтроку(Строка) Экспорт
	...
	Если ПустаяСтрока(ОписаниеТипаСтрока(0).ПривестиЗначение(Строка)) Тогда
		Возврат "";
	Иначе
		Возврат XMLЗначение(Тип("ХранилищеЗначения"), Строка).Получить();
	КонецЕсли;
...
Процедура СообщитьПользователю(Знач ТекстСообщенияПользователю, Знач КлючДанных = Неопределено,	Знач Поле = "",
	...
	Если СообщитьСообщение Тогда
		...
		Сообщение = SUI_ОбщегоНазначенияСлужебныйКлиентСервер.СообщениеПользователю(ТекстСообщенияПользователю,
...
// Предназначена для вызова необязательной подсистемы (условного вызова) совместно с SUI_ОбщегоНазначения.ОбщийМодуль.
//
// У функциональной подсистемы снят флажок "Включать в командный интерфейс".
// и SUI_ОбщегоНазначенияКлиент.ПодсистемаСуществует для вызова из клиентского кода.
//
// Параметры:
...
//
// Пример:
//  Если SUI_ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
//  	МодульВариантыОтчетов = SUI_ОбщегоНазначения.ОбщийМодуль("ВариантыОтчетов");
...
Функция ПодсистемаСуществует(ПолноеИмяПодсистемы) Экспорт
	...
	ИменаПодсистем = SUI_СтандартныеПодсистемыПовтИсп.ИменаПодсистем();
...
Функция ИнформационнаяБазаФайловая(Знач СтрокаСоединенияИнформационнойБазы = "") Экспорт
	...
	Если ПустаяСтрока(СтрокаСоединенияИнформационнойБазы) Тогда
		Возврат SUI_СтандартныеПодсистемыПовтИсп.ИнформационнаяБазаФайловая();
	КонецЕсли;
...
Функция ОбъектМетаданныхПоПолномуИмени(ПолноеИмя) Экспорт
	...
	ИменаКоллекций = SUI_СтандартныеПодсистемыПовтИсп.ИменаКоллекцийПоИменамБазовыхТипов();
...
//
// Параметры:
//  Тип - Тип, ОписаниеТипов - для которого надо получить представление.
//
// Возвращаемое значение:
//  Строка - строковое представление типа или типов
Функция СтроковоеПредставлениеТипа(Тип) Экспорт
	
	ПредставлениеТипа = Новый Массив();
	
	ТипЗначенияТипа = ТипЗнч(Тип);
	
	Если ТипЗначенияТипа = Тип("Неопределено") Тогда
		ПредставлениеТипа.Добавить("Неопределено");
		
	ИначеЕсли ТипЗначенияТипа = Тип("Строка") Тогда
		ПредставлениеТипа.Добавить("Строка");

	ИначеЕсли ТипЗначенияТипа = Тип("Число") Тогда
		ПредставлениеТипа.Добавить("Число");

	ИначеЕсли ТипЗначенияТипа = Тип("Булево") Тогда
		ПредставлениеТипа.Добавить("Булево");

	ИначеЕсли ТипЗначенияТипа = Тип("Дата") Тогда
		ПредставлениеТипа.Добавить("Дата");
		
	ИначеЕсли ТипЗначенияТипа = Тип("ОписаниеТипов") Тогда
		
		ТипыОписанияТипов = Тип.Типы();
		
		Для индекс = 0 По ТипыОписанияТипов.ВГраница() Цикл
			ПредставлениеТипа.Добавить(СтроковоеПредставлениеТипа(ТипыОписанияТипов[индекс]));
		КонецЦикла;
	...
	ИначеЕсли ТипЗначенияТипа = Тип("Тип") Тогда
		...
		Если ТипXML <> Неопределено И СтрНайти(ТипXML.ИмяТипа, ".") > 0 Тогда
			...
			ЧастиТипXML = СтрРазделить(ТипXML.ИмяТипа, ".");
			
			Если ЧастиТипXML.Количество() > 1 Тогда
				
				Если ЧастиТипXML[0] = "CatalogRef" Тогда
					Результат = "СправочникСсылка";
				ИначеЕсли ЧастиТипXML[0] = "CatalogObject" Тогда
					Результат = "СправочникОбъект";
				ИначеЕсли ЧастиТипXML[0] = "DocumentRef" Тогда
					Результат = "ДокументСсылка";
				ИначеЕсли ЧастиТипXML[0] = "DocumentObject" Тогда
					Результат = "ДокументОбъект";
				ИначеЕсли ЧастиТипXML[0] = "BusinessProcessRef" Тогда
					Результат = "БизнесПроцессСсылка";
				ИначеЕсли ЧастиТипXML[0] = "BusinessProcessObject" Тогда
					Результат = "БизнесПроцессОбъект";
				ИначеЕсли ЧастиТипXML[0] = "BusinessProcessRoutePointRef" Тогда
					Результат = "ТочкаМаршрутаБизнесПроцессаСсылка";
				ИначеЕсли ЧастиТипXML[0] = "BusinessProcessRoutePointObject" Тогда
					Результат = "ТочкаМаршрутаБизнесПроцессаОбъект";
				ИначеЕсли ЧастиТипXML[0] = "ChartOfCharacteristicTypesRef" Тогда
					Результат = "ПланВидовХарактеристикСсылка";
				ИначеЕсли ЧастиТипXML[0] = "ChartOfCharacteristicTypesObject" Тогда
					Результат = "ПланВидовХарактеристикОбъект";
				ИначеЕсли ЧастиТипXML[0] = "TaskRef" Тогда
					Результат = "ЗадачаСсылка";
				ИначеЕсли ЧастиТипXML[0] = "TaskObject" Тогда
					Результат = "ЗадачаОбъект";
				ИначеЕсли ЧастиТипXML[0] = "ExchangePlanRef" Тогда
					Результат = "ПланОбменаСсылка";
				ИначеЕсли ЧастиТипXML[0] = "ExchangePlanObject" Тогда
					Результат = "ПланОбменаОбъект";
				ИначеЕсли ЧастиТипXML[0] = "EnumRef" Тогда
					Результат = "ПеречислениеСсылка";
				КонецЕсли;
				
				Если Не ПустаяСтрока(Результат) Тогда
					Результат = Результат + "." + ЧастиТипXML[1];
				КонецЕсли;
		...
		ПредставлениеТипа.Добавить(Результат);
		
	Иначе
		SUI_ОбщегоНазначенияКлиентСервер.Проверить(Ложь,
			НСтр("ru = 'Недопустимое значение параметра (параметр номер ""1"")'"),
			"SUI_ОбщегоНазначения.СтроковоеПредставлениеТипов");
	КонецЕсли;
	
	Если ПредставлениеТипа.Количество() = 1 Тогда
		Возврат ПредставлениеТипа[0];
	Иначе
		Возврат СтрСоединить(ПредставлениеТипа, "; ");
	КонецЕсли;
