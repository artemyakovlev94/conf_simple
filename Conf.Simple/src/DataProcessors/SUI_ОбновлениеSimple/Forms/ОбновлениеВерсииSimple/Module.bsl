...
Процедура УстановитьВерсиюДанныхНаСервере(НомерВерсииДанных)
	SUI_ОбновлениеSimple.УстановитьВерсиюДанных(
		SUI_ОбщегоНазначения.ОписаниеТипаСтрока(12).ПривестиЗначение(НомерВерсииДанных));
КонецПроцедуры
...
Процедура ОбновитьИнформационнуюБазуПриОшибкеУстановкиМонопольногоРежима(ДополнительныеПараметры)
	...
	Если Не SUI_ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЗавершениеРаботыПользователей") Тогда
	...
	МодульСоединенияИБКлиент = SUI_ОбщегоНазначенияКлиент.ОбщийМодуль("СоединенияИБКлиент");
