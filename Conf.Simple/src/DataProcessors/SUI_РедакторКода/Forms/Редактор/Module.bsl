...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаРедактор.Подсказка = Параметры.Описание;
	...
	Если ПолучитьФункциональнуюОпцию("SUI_ИспользоватьРедакторКодаBSL") Тогда
	...
	Если ТипЗнч(Параметры.ПользовательскиеОбъекты) = Тип("Структура") Тогда
		СформироватьДеревоОписанияПользовательскихОбъектов(Параметры.ПользовательскиеОбъекты, ЭлементыДереваОписанияПеременных);
	КонецЕсли;
	Элементы.ФормаОписаниеПеременных.Видимость = (ЭлементыДереваОписанияПеременных.Количество() > 0);
	
	Элементы.ФормаПредопределенныйКод.Видимость = Не ПустаяСтрока(ПредопределенныйТекст);
...
Процедура ПриОткрытии(Отказ)
	...
	СменитьОтображениеДереваОписанияПеременных(Элементы.ФормаОписаниеПеременных.Видимость);
...
Процедура РедакторBSLДокументСформирован(Элемент)
	...
	Если БиблиотекаBSLЗагружена Тогда
		...
		#Если ВебКлиент Тогда
			...
			Если ВебДокументРедактораBSLДоступен() Тогда
...
&НаКлиенте
Процедура ОписаниеПеременных(Команда)
	
	СменитьОтображениеДереваОписанияПеременных();
...
Процедура ПредопределенныйКод(Команда)
	...
	Если ПустаяСтрока(ПредопределенныйТекст) Или Не РедактированиеДоступно() Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ИнициализироватьБиблиотеку(ЭтоПереустановка = Ложь)
	
	Контекст = Новый Структура();
	
	Если ЭтоПереустановка Тогда
		Контекст.Вставить("ПереустановитьБиблиотеку");
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ПослеПодключенияРасширенияДляРаботыСФайламиИнициализироватьБиблиотеку", ЭтотОбъект, Контекст);
	НачатьПодключениеРаботыСФайлами(Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура НачатьПодключениеРаботыСФайлами(ОповещениеЗавершения)
	
	ТекстПредложения = НСтр("ru = 'Для работы с редактором кода необходимо установить расширение для работы с файлами.'");
	SUI_ОбщегоНазначенияКлиент.ПодключитьРасширениеРаботыСФайламиСВозможнойУстановкой(ОповещениеЗавершения, ТекстПредложения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеПодключенияРасширенияДляРаботыСФайламиИнициализироватьБиблиотеку(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ПоказатьОшибку(НСтр("ru = 'Расширение для работы с файлами не подключено.'"), Истина);
	КонецЕсли;
...
Процедура НачатьПроверкуСуществованияИндексногоФайлаБиблиотеки(Контекст)
	...
	Контекст.Вставить("ПутьИндексногоФайла", SUI_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогБиблиотеки, "index.html"));
	
	Оповещение = Новый ОписаниеОповещения("ПослеПроверкиСуществованияИндексногоФайлаБиблиотеки", ЭтотОбъект, Контекст);
	ФайлНаДиске = Новый Файл(Контекст.ПутьИндексногоФайла);
	ФайлНаДиске.НачатьПроверкуСуществования(Оповещение);
...
Процедура ПослеПроверкиСуществованияИндексногоФайлаБиблиотеки(Существует, Контекст) Экспорт
	...
	ПутьИндексногоФайла = "";
	Если Контекст.Свойство("ПутьИндексногоФайла", ПутьИндексногоФайла) Тогда
		Контекст.Удалить("ПутьИндексногоФайла");
	Иначе
		ПоказатьОшибку(НСтр("ru = 'Ошибка параметров контекста (ПутьИндексногоФайла).'"), Истина);
	КонецЕсли;
	...
	Если Существует Тогда
		...
		НачатьКопированиеФайла(Оповещение, ПутьИндексногоФайла, ИндексныйФайл());
...
Функция ПолучитьМодульИзИсходников(ПолноеИмяФайла)
	...
	#Если ВебКлиент Тогда
		//TODO: 13.09.2024 - artem - Реализовать чтение на веб клиенте
	#Иначе
	...
	Для Каждого Элемент Из КоллекцияПользовательскихОбъектов Цикл
		...
		Если Элемент.Значение.Свойство("ref") Тогда
			...
			Если _XMLТип <> Неопределено И 
				SUI_ОбщегоНазначенияПереопределяемый.ЭтоСсылка(Элемент.Значение.ref) Тогда
		...
		Если Элемент.Значение.Свойство("type") Тогда
			...
			ПредставлениеТипа = SUI_ОбщегоНазначенияПереопределяемый.СтроковоеПредставлениеТипа(Элемент.Значение.type);
...
Функция ПривестиПользовательскиеОбъектыКСтрокеJSON(Знач КоллекцияПользовательскихОбъектов)
	...
	Если ТипЗнч(КоллекцияПользовательскихОбъектов) = Тип("Структура") Тогда
		...
		Результат = ОбщегоНазначения.СкопироватьРекурсивно(КоллекцияПользовательскихОбъектов, Ложь);
...
Процедура СформироватьДеревоОписанияПользовательскихОбъектов(КоллекцияПользовательскихОбъектов, ЭлементыДерева = Неопределено)
	...
	Для Каждого ЭлементКоллекции Из КоллекцияПользовательскихОбъектов Цикл
		...
		Если ЭлементКоллекции.Значение.Свойство("type") Тогда
			...
			НовыйЭлементДерева.Тип = SUI_ОбщегоНазначенияПереопределяемый.СтроковоеПредставлениеТипа(ЭлементКоллекции.Значение.type);
...
&НаКлиенте
Процедура СменитьОтображениеДереваОписанияПеременных(Значение = Неопределено)
	
	Если Значение = Неопределено Тогда
		Элементы.ФормаОписаниеПеременных.Пометка = Не Элементы.ФормаОписаниеПеременных.Пометка;
	Иначе
		Элементы.ФормаОписаниеПеременных.Пометка = Значение;
	КонецЕсли;
	
	Элементы.ГруппаРедакторОписаниеПеременных.Видимость = Элементы.ФормаОписаниеПеременных.Пометка;
...
Процедура ВставитьПредопределенныйКодПодтверждение(Подтверждение, Контекст) Экспорт
	...
	Если Контекст.ВРедакторBSL Тогда
		ОчиститьТекстРедактораBSL();
		УстановитьТекстРедактораBSL(ПредопределенныйТекст, Неопределено, Ложь);
	Иначе
		РедактируемаяСтрока = ПредопределенныйТекст;
	КонецЕсли;
